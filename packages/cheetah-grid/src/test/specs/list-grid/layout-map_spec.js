...
(function() {
	describe('layout maps', function() {
		it('builds simple header layout maps with grouped headers and empty fallback data', async function() {
			const {SimpleHeaderLayoutMap} = await import('../../../js/list-grid/layout-map/internal/simple-header-layout.ts');
			const map = new SimpleHeaderLayoutMap([
				{
					caption: 'Group',
					columns: [
		...
		it('builds multi layout maps with row and column spans', async function() {
			const {MultiLayoutMap} = await import('../../../js/list-grid/layout-map/internal/multi-layout.ts');
			const map = new MultiLayoutMap({
				header: [
					[
