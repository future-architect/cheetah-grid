...
(function() {
		...
		header: [
			{
				...
				caption: 'style',
				width: 150,
				style: {indicatorBottomRight: {style: 'triangle'}},
			},
			{
				field: 'bool',
				caption: 'function',
				width: 150,
				style: (rec) => ({
					indicatorBottomRight: {style: rec.bool ? 'triangle' : undefined},
				}),
			},
			{
				field: 'bool',
				...
				width: 150,
				style: {indicatorBottomRight: 'triangle'},
			},
		],
		records: records,
	});
	...
	window.grid = grid;
	const theme = {indicators: {bottomRightColor: undefined, bottomRightSize: undefined}};
	grid.theme = cheetahGrid.themes.choices.MATERIAL_DESIGN.extends(theme);

	describe('indicatorBottomRight', function() {
		...
		it('init drawing', function(done) {
			function createAnswerCanvas() {
				...
				for (const cell of [
					...
					drawBottomRightTriangleIndicator({
						ctx,
						color: BORDER_COLOR,
		...
		it('update color', function() {
			const color = '#00F';
			theme.indicators.bottomRightColor = color;
			grid.invalidate();
		...
		it('reset color', function() {
			theme.indicators.bottomRightColor = undefined;
			grid.invalidate();
		...
		it('update size', function() {
			const size = 6;
			theme.indicators.bottomRightSize = size;
			grid.invalidate();
		...
		it('reset size', function() {
			theme.indicators.bottomRightSize = undefined;
			grid.invalidate();
...
function drawBottomRightTriangleIndicator({ctx, rect, color, size}) {
	ctx.fillStyle = color;
	ctx.beginPath();
	ctx.moveTo(rect.right - 2, rect.bottom - 2);
	ctx.lineTo(rect.right - 2 - size, rect.bottom - 2);
	ctx.lineTo(rect.right - 2, rect.bottom - 2 - size);
	ctx.closePath();
	ctx.fill();
}
